...
3. At the start, create and read `LOOP_SKILL_MEMORY.md` at the project root; if it already exists, ask the user before proceeding. Keep it limited to information that implementation and fix subagents must know, update it only when such information exists, and require those subagents to read it before working. Keep the file untracked and exclude its path from every review diff, staging, commit, and PR without deleting the file. Do not delete, remove, prune, or otherwise clean up either the task worktree or `LOOP_SKILL_MEMORY.md` unless the user explicitly requests that deletion. PR creation, closure, merge, and workflow completion do not authorize cleanup. Require review-only subagents to avoid all access to `LOOP_SKILL_MEMORY.md`.
