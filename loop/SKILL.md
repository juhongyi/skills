...
3. At the start, create and read a temporary `LOOP_SKILL_MEMORY.md` at the project root for use only while the Loop workflow runs; if it already exists, ask the user before proceeding. Keep it limited to information that implementation and fix subagents must know, update it only when such information exists, and require those subagents to read it before working. Never include it in a review diff, commit, or PR. Require review-only subagents to avoid all access to `LOOP_SKILL_MEMORY.md`. Do not delete either the worktree or `LOOP_SKILL_MEMORY.md` unless the user explicitly directs deletion.
